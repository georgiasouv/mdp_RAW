...
#SBATCH --job-name=eval_solo_T_normgrad
#SBATCH --output=/networkhome/WMGDS/souval_g/raw-mdp/cluster_scripts/logs/eval_solo_T_normgrad_%j.out
#SBATCH --error=/networkhome/WMGDS/souval_g/raw-mdp/cluster_scripts/logs/eval_solo_T_normgrad_%j.err
# ============================================================
#  eval solo_T_normgrad  ->  vs fcos, fasterrcnn, detr, retinanet
# ============================================================
...
export WANDB_RUN_GROUP=eval__solo_T_normgrad
...
PREP=runs/pascalraw__solo_T_normgrad/preprocessor.pth
...
echo "=== Evaluating solo_T_normgrad against all detectors ==="
for DET in fcos fasterrcnn detr retinanet; do
    echo "--- solo_T_normgrad vs $DET ---"
    python -m core.evaluate \
        --prep "$PREP" \
        --detector "$DET" \
        --dataset pascalraw \
        --num-classes 3 \
        --out results/eval_normgrad/solo_T_normgrad__${DET}.json \
        --wandb \
        --wandb-name solo_T_normgrad__eval_${DET}
done
...
echo "=== eval solo_T_normgrad finished ==="
